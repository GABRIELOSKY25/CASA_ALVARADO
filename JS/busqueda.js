    ...
    try {
        ...
        document.getElementById('grid-productos').innerHTML =
            `<div class="mensaje-vacio">
                ...
                ¿FastAPI está corriendo en ${API_BASE}?
            </div>`;
...
async function generarFiltrosDesdeProductos() {
    try {
        ...
        todasLasMarcas = marcas.map(nombre => ({
            nombre,
            imagen: null
        }));
        ...
        document.getElementById('filtros-loading').innerHTML =
            ...
            <div class="filtro-contenido">
                ${marcas.map(marca => `
                    <label class="opcion_filtro">
                        ...
                        <span
                            class="contador"
                            id="contador-marca-${marca.replace(/ /g, '-')}"
                        >
                            (0)
                        </span>
                    </label>
            ...
            <div class="filtro-contenido">
                ${tipos.map(tipo => `
                    <label class="opcion_filtro">
                        ...
                        <span
                            class="contador"
                            id="contador-tipo-${tipo.replace(/ /g, '-')}"
                        >
                            (0)
                        </span>
                    </label>
            ...
            <div class="filtro-contenido">
                ${gamas.map(gamma => `
                    <label class="opcion_filtro">
                        ...
                        <span
                            class="contador"
                            id="contador-gamma-${gamma.replace(/ /g, '-')}"
                        >
                            (0)
                        </span>
                    </label>
...
function aplicarFiltros() {
    ...
    document.querySelectorAll('.carrusel-item')
        .forEach(i => i.classList.remove('activo'));

    const marcasSeleccionadas = Array.from(
        document.querySelectorAll('.filtro-marca:checked')
    ).map(cb => cb.value);
    ...
    const gamasSeleccionadas = Array.from(
        document.querySelectorAll('.filtro-gamma:checked')
    ).map(cb => cb.value);

    if (
        marcasSeleccionadas.length === 0 &&
        tiposSeleccionados.length === 0 &&
        gamasSeleccionadas.length === 0
    ) {
    ...
    if (tiposSeleccionados.length) {
        productosFiltrados = productosFiltrados.filter(
            p => tiposSeleccionados.includes(p.tipo)
        );
    }

    if (gamasSeleccionadas.length) {
        productosFiltrados = productosFiltrados.filter(
            p => gamasSeleccionadas.includes(p.gamma)
        );
    }
...
function actualizarContadores(productos) {
    ...
    Object.entries(marcaCount).forEach(([nombre, total]) => {
        const idLimpio = nombre.replace(
            /[&/\\#,+()$~%.'":*?<>{}]/g,
            '-'
        );

        const span = document.querySelector(
            `#contador-marca-${idLimpio}`
        );
...
function renderizarProductos(productos) {
    ...
    if (!productos.length) {
        grid.innerHTML =
            '<div class="mensaje-vacio">⚠️ No hay productos con los filtros seleccionados</div>';
    ...
    const estrellasHTML = (calificacion) => {
        ...
        const colorEstrella = estrellas > 0
            ? '#ffc107'
            : '#ccc';
    ...
    grid.innerHTML = productos.map(producto => `
        ...
        >
            ...
            <a
                href="producto.html?modelo=${encodeURIComponent(producto.modelo)}"
    ...
    trackParaFiltros.addEventListener('click', (e) => {
        ...
        if (item) {
            ...
            if (marcaSeleccionada) {
                ...
                const checkboxesMarcas =
                    document.querySelectorAll('.filtro-marca');

                checkboxesMarcas.forEach(cb => {
                    cb.checked =
                        cb.value === marcaSeleccionada;
                });

                document.querySelectorAll(
                    '.filtro-tipo, .filtro-gamma'
...
function inicializarBuscador() {
    ...
    buscador.addEventListener('input', function(e) {
        ...
        if (terminoBusqueda === '') {
            document.querySelectorAll(
                '.filtro-marca, .filtro-tipo, .filtro-gamma'
            ).forEach(cb => cb.checked = false);
        ...
        timeoutBuscador = setTimeout(async () => {
            try {
                const response = await fetch(
                    `${API_BASE}/productos/buscar/${encodeURIComponent(terminoBusqueda)}`
                );
                ...
                grid.innerHTML =
                    '<div class="error-mensaje">❌ Error al buscar productos</div>';
            }
        }, 300);
    });
}
